...
语法分析的过程是一个递归下下降的过程，是自上而下过程的一种实现。<br/>
...
stat: 'if'
	...
	;
```
antlr采取的策略是优先匹配靠前的备选分支，比如说，对于语句if，antlr会匹配第一个备选分支'if'，而不是[a-zA-Z]+<br/>
...
这一点在我们生成语法分析树时很有用，antlr会根据标签名生成对应的节点类和方法。<br/>
我们可以更准确地知道匹配到了哪一条规则。<br/>
<br/>
...
mul: '(' expr ('*' |'/') expr ')'
```
经典的自上而下的语法分析器无法处理左递归，antlr经过改进，得以处理直接左递归，但对间接右递归还是无能为力。<br/>
左递归：在某个备选分支的最左侧以直接或间接方式调用了自身。<br/>
直接左递归(前一个)，间接左递归(后一个)。<br/>
