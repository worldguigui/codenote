...
## 八.纯 λ 演算中的常量
如何在一个只有函数的系统（纯λ演算）中，定义我们熟悉的基本数据（如数字、布尔值、列表）和操作（如加法、判断）呢？<br/>
这一节会给我们答案：用函数编码一切。<br/>

### 1. 布尔值的编码

TRUE = λa.λb. a<br/>
含义：给定两个选项，总是选择第一个。<br/>

FALSE = λa.λb. b<br/>
含义：给定两个选项，总是选择第二个。<br/>

条件判断 IF 就可以用如下的λ表达式表示：<br/>
IF = λc.λt.λf. c t f<br/>

如果 c 是 TRUE，返回 t；如果 c 是 FALSE，返回 f。<br/>

### 2. 自然数的编码（Church 数字）
用函数应用的次数来表示数字，因为在前面的运算中，我们是预定义形如‘1’的符号，代表数字1，而没有用函数表示出来。<br/>
...
n = λf.λx. f (f (... (f x)...)) （f 出现 n 次）<br/>

后继函数 SUCC 的定义体现了这种编码的优雅：<br/>
SUCC = λn.λf.λx. f (n f x)<br/>
解释：对于数字 n（它已经表示“应用 f n 次”），再额外应用 f 一次，总共就是 n+1 次。<br/>

加法 ADD 的定义：<br/>
ADD = λm.λn.λf.λx. m f (n f x)<br/>
解释：先对 x 应用 f n 次（得到 n 的结果），再对这个结果应用 f m 次。总共应用 m+n 次。<br/>

### 3. 数据结构的编码（序对和列表）
通过函数来“记住”两个值，实现最基本的数据结构。<br/>

PAIR（构造器）：λa.λb.λf. f a b<br/>
它接收两个值 a 和 b，返回一个函数 λf. f a b，这个函数“知道” a 和 b。<br/>

HEAD（选择器）：λp. p (λa.λb. a)<br/>
它将一个“总是返回第一个参数”的函数传给 PAIR，PAIR 就会交出 a。<br/>

TAIL（选择器）：λp. p (λa.λb. b)<br/>
原理同上，传递“总是返回第二个参数”的函数。<br/>

有了 PAIR 和一个表示空列表的常量 NIL，就可以构建任意长度的列表，例如 [1,2,3] = PAIR 1 (PAIR 2 (PAIR 3 NIL))。<br/>
